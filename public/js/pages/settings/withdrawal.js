...
$(function () {
    ...
    $('#withdrawal-payout-account-id, #withdrawal-payment-identifier, #withdrawal-message').on('change keyup', function () {
        $(this).removeClass('is-invalid');
    });
...
var Wallet = {
    ...
    shouldPersistMethodDetails: function () {
        return ['paypal', 'crypto', 'custom'].includes(Wallet.getSelectedProvider());
    },
    ...
    rememberCurrentMethodDetails: function () {
        ...
        if (!['paypal', 'crypto', 'custom'].includes(methodKey)) {
            return;
        }
