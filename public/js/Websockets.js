...
soketi, socketsDriver, messenger, FileUpload, videoPreview, trans, StoryCreator,
incrementNotificationsCount, launchToast, ReelCreator */
...
var Websockets = {
    ...
    bindPrivateChannelEvents: function (channel, location) {
        ...
        // Only bind video-processing on pages using the shared FileUpload uploader.
        const handlesVideoProcessing = (
            location.indexOf('posts/create') >= 0 ||
            location.indexOf('posts/edit') >= 0 ||
            location.indexOf('stories/create') >= 0 ||
            location.indexOf('reels/create') >= 0
        );
        if (handlesVideoProcessing) {
                ...
                FileUpload.myDropzone.files.forEach((file) => {
                    if (file.upload.attachmentID === data.id) {
                        ...
                        if (data.success) {
                            ...
                            if (typeof StoryCreator !== 'undefined' && String(StoryCreator.attachmentID) === String(data.id)) {
                                StoryCreator.setActiveAttachment(data.id);
                            }
                            if (typeof ReelCreator !== 'undefined' && typeof ReelCreator.updatePreview === 'function') {
                                ReelCreator.updatePreview();
                            }
                        } else {
                            FileUpload.myDropzone.removeFile(file);
