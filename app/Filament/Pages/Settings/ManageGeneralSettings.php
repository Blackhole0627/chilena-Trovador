...
class ManageGeneralSettings extends SettingsPage
{
    ...
    {
        if (($data['enforce_user_identity_checks'] ?? false) && ($data['hide_identity_checks'] ?? false)) {
            $data['hide_identity_checks'] = false;
        }
                ...
                ->tabs([
                    Tab::make('Site')
                        ->schema([
                            ...
                            Toggle::make('enforce_user_identity_checks')
                                ...
                                ->helperText('Requires ID verification before users can publish content or start streams.')
                                ->live()
                                ->afterStateUpdated(function (Set $set, bool $state): void {
                                    if ($state) {
                                        $set('hide_identity_checks', false);
                                    }
                                }),

                            Toggle::make('hide_identity_checks')
                                ->label('Hide identity checks')
                                ->helperText('Removes ID verification from user settings. Intended for sites that do not require creator verification.')
                                ->live()
                                ->afterStateUpdated(function (Set $set, bool $state): void {
                                    if ($state) {
                                        $set('enforce_user_identity_checks', false);
                                    }
                                }),
                    ...
                    Tab::make('Explore page')
                        ->schema([
                            ...
                            Toggle::make('explore_follow_button_enabled')
                                ->label('Show follow button in people search')
                                ->helperText('Lets users follow or unfollow creators directly from the Explore people results.')
                                ->default(true)
                                ->live(),
                        ])
                        ->columns(2),
