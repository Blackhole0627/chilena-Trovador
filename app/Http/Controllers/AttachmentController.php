...
class AttachmentController extends Controller
{
    ...
    public function upload(UploadAttachamentRequest $request, $type = false, $chunkedFile = false)
    {
        ...
        try {
            ...
            if ($type == 'message' && in_array($fileExtension, AttachmentServiceProvider::getRecordedAudioExtensions())) {
                $directory = 'audio';
            }
            if ($type == 'post') {
    ...
    public static function handleCoconutHook(Request $request) {
        ...
        if ($request->get('event') === 'job.completed') {
            ...
            $isAudio = AttachmentServiceProvider::getAttachmentType($attachment->type) === 'audio';
            $baseDirectory = $isAudio ? 'messenger/audio' : 'posts/videos';
            if (str_contains($attachment->filename, '/tmp/')) {
                $baseDirectory = trim(dirname(dirname($attachment->filename)), '/');
            }

            $extension = $isAudio ? $attachment->type : 'mp4';
            $attachment->filename = "{$baseDirectory}/{$attachmentID}.{$extension}";
            ...
            if (!$duration) {
                ...
                foreach ($streams as $s) {
                    ...
                    if (($s['codec_type'] ?? null) === $streamType && !empty($s['duration'])) {
                        $duration = $s['duration'];
                        break;
                    }
                }
            ...
            if (config('broadcasting.connections.pusher.key')) {
                ...
                self::triggerVideoProcessingNotification($username, $attachment);
            }
        }
        ...
        return response()->json(['success' => true, 'message' => __("Media updated")], 200);

    }

    /**
     * Sends a best-effort websocket update for Coconut processing status.
     */
    private static function triggerVideoProcessingNotification(string $username, Attachment $attachment): void
    {
        try {
            ...
            Log::channel('coconut')->error('Failed to publish video processing websocket update', [
                'username' => $username,
