    ...
    <a href="{{$attachment->path}}" rel="mswp" title="" class="no-long-press">
        ...
        @if($isGallery)
            @if(AttachmentHelper::getAttachmentType($attachment->type) == 'image')
                <img src="{{$attachment->path}}" draggable="false" alt="" class="post-media-gallery-image">
            @elseif(AttachmentHelper::getAttachmentType($attachment->type) == 'video')
                <div class="video-wrapper post-media-gallery-video-wrapper h-100 w-100 d-flex justify-content-center align-items-center">
