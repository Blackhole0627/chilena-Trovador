<div class="post-box" data-postID="{{$post->id}}">
        ...
        <div class="d-flex">
            ...
            <div class="post-details pl-2 w-100{{$post->is_pinned ? '' : '' }}">
                <div class="d-flex justify-content-between">
                    <div>
                        <div class="text-bold max-width-150 text-truncate d-flex align-items-center">
                            <a href="{{route('profile',['username'=>$post->user->username])}}" class="text-dark-r">{{$post->user->name}}</a>
                            @if(GenericHelper::isUserVerified($post->user))
                    ...
                    <div class="d-flex">
                        ...
                        <div class="pr-3 pr-md-3">
                            <a class="text-dark-r text-hover d-flex text-muted" onclick="PostsPaginator.goToPostPageKeepingNav({{$post->id}},{{$post->postPage}},'{{route('posts.get',['post_id'=>$post->id,'username'=>$post->user->username])}}')" href="javascript:void(0)">
    ...
    <div class="post-content mt-3 {{count($post->attachments) ? "mb-3" : ""}} pl-3 pr-3">
        <div class="text-break post-content-data {{ getSetting('feed.enable_post_description_excerpts') ? 'line-clamp-3' : '' }}">
            @if(PostsHelper::shouldHidePostText($post))
                ...
                @endif
            @else
                {!! PostsHelper::renderPostText($post) !!}
            @endif
        </div>
            ...
            @if(PostsHelper::isPostSubscriptionUnlocked($post))
                ...
                @else
                    @if(count($post->attachments) > 1)
                        <div class="swiper mySwiper pointer-cursor post-media-swiper {{ getSetting('feed.post_box_max_height') ? 'post-media-swiper-fixed-height' : 'post-media-swiper-native-height' }}">
            ...
            <div class="d-flex">
                ...
                @if($post->isSubbed || (Auth::check() && \App\Providers\ProfileMonetizationServiceProvider::userHasOpenProfile($post->user)))
                    <div class="h-pill h-pill-primary mr-1 rounded react-button {{PostsHelper::didUserReact($post->reactions) ? 'active' : ''}}" data-toggle="tooltip" data-placement="top" title="{{__('Like')}}" onclick="Post.reactTo('post',{{$post->id}})">
                        <span class="reaction-icon-active {{PostsHelper::didUserReact($post->reactions) ? '' : 'd-none'}}">
                            @include('elements.icon',['icon'=>'heart', 'variant'=>'medium', 'classes'=>'text-primary'])
                        </span>
                        <span class="reaction-icon-inactive {{PostsHelper::didUserReact($post->reactions) ? 'd-none' : ''}}">
    ...
    @if($post->isSubbed || (Auth::check() && \App\Providers\ProfileMonetizationServiceProvider::userHasOpenProfile($post->user)))
        <div class="post-comments d-none" {{Route::currentRouteName() == 'posts.get' ? 'id="comments"' : ''}}>
            ...
            <div class="no-comments-label d-none">
                @include('elements.comments-empty-state')
            </div>
            @if(Auth::check())
                <hr>
